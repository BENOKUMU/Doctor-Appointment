...
$(document).ready(function(){
	...
	$(document).ready(function() {
			...
			// Retrieve time slots from action.php via AJAX
			$.ajax({
			  url: "action.php",
			  method: "POST",
			  data: { action: "get_time_slots", doctor_id: doctor_id },
			  dataType: "json",
			  success: function (data) {
			    // Generate dropdown options
			    var dropdown = $('#timeslot');
			    dropdown.empty();
			    dropdown.append($('<option>').val('').text('-- Please select a time slot --'));
			    $.each(data, function (index, slot) {
			      dropdown.append($('<option>').val(slot.start_time).text(slot.start_time + ' - ' + slot.end_time));
			    });
			  }
			});
		});
