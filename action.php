...
if(isset($_POST["action"]))
{
		...
		if($object->row_count() > 0)
		{
			...
			foreach($result as $row)
			{
				if($row["email_verify"] == 'Yes')
				{
					...
					if (password_verify($_POST["patient_password"], $result[0]['patient_password']))
					{
	...
	if($_POST['action'] == 'fetch_schedule')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="get_appointment" class="btn btn-primary btn-sm get_appointment" data-doctor_id="'.$row["doctor_id"].'" data-doctor_schedule_id="'.$row["doctor_schedule_id"].'">Book Appointment</button>
			</div>
			';
			$data[] = $sub_array;
		}
	...
	if($_POST['action'] == 'make_appointment')
{
   ...
   foreach ($doctor_schedule_data as $doctor_schedule_row) {
    $start_time = strtotime($doctor_schedule_row["doctor_schedule_start_time"]);
    $end_time = strtotime($doctor_schedule_row["doctor_schedule_end_time"]);
    $avg_consult_period = $doctor_schedule_row["average_consulting_time"];

    $num_time_slots = ceil(($end_time - $start_time) / ($avg_consult_period * 60));

    $time_slot_array = array();
    $curr_time = $start_time;
    for ($i = 0; $i < $num_time_slots; $i++) {
        $time_slot_array[] = array(
            "start_time" => date("H:i", $curr_time),
            "end_time" => date("H:i", strtotime("+".$avg_consult_period." minutes", $curr_time))
        );
        $curr_time = strtotime("+".$avg_consult_period." minutes", $curr_time);
    }
    ...
    $html .= '
        ...
        <tr>
            <th width="40%" class="text-right">Select Time Slot</th>
            <td>
                <select id="appointment_time_select" name="appointment_time_select" class="form-control">
                    <option value="">Select a time slot</option>';
					foreach ($time_slot_array as $time_slot) {
						$html .= '<option value="'.$time_slot["start_time"].'">'.$time_slot["start_time"].' - '.$time_slot["end_time"].'</option>';
					}
					$html .= '
                </select>
            </td>
