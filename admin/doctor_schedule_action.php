	...
	if($_POST["action"] == 'fetch')
	{
		...
		if($_SESSION['type'] == 'Admin')
		{
			...
			if(isset($_POST["search"]["value"]))
			{
				...
				// $search_query .= 'OR doctor_schedule_table.num_slots LIKE "%'.$_POST["search"]["value"].'%" ';
				// $num_slots = $_POST['num_slots'];
// $sql = "INSERT INTO schedule (doctor_id, date, start_time, end_time, num_slots) VALUES ('$doctor_id', '$date', '$start_time', '$end_time', '$num_slots')";

			}
		}
		else
		{
			$order_column = array('doctor_schedule_date', 'doctor_schedule_day', 'doctor_schedule_start_time', 'doctor_schedule_end_time', 'average_consulting_time', 'num_slots');
			$main_query = "
			SELECT * FROM doctor_schedule_table 
			";
			...
			if(isset($_POST["search"]["value"]))
			{
				...
				// $search_query .= 'OR num_slots LIKE "%'.$_POST["search"]["value"].'%") ';
				// $num_slots = $_POST['num_slots'];
			}
		}
